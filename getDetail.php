...
$id=$id - 1;
$sql="select title,id from qiangge_article where id='{$id}'";
$pre_row=fetchOne($sql, $link);
$id=$id + 1;
$sql="select title,id from qiangge_article where id='{$id}'";
$next_row=fetchOne($sql, $link);
$sql="select title,id from qiangge_article where cate='{$a_row['cate']}'";
$like_rows=fetchAll($link, $sql);
...
<head>
<meta charset="utf-8">
<title><?php echo $a_row["title"]?></title>
<meta name="keywords" content="<?php echo $a_row['title'];echo $a_row['cate']?>" />
...
<div class="ibody">
   ...
   <article>
    <h2 class="about_h">您现在的位置是：博客详情页</h2>
    <div class="Qt">
    <h1><?php echo $a_row['title'];?></h1>
    <p class="box_c"><span class="d_time">发布时间：<?php echo $a_row['createtime'];?></span><span>编辑：<?php echo $a_row['author'];?></span><span>浏览（390）</span><span>评论览（14）</span></p>
<p>概要：<?php echo $a_row['brief_introduction'];?></p>
<br/><br/>
<?php echo $a_row['mText'];?>
</div>
      <div class="nextinfo">
        <p>上一篇：<a href="getDetail.php?id=<?php echo $a_row['id']?>"><?php echo $pre_row['title']?></a></p>
        <p>下一篇：<a href="getDetail.php?id=<?php echo $a_row['id']?>"><?php echo $next_row['title']?></a></p>
      </div>
  </article>
  <aside>
    <div class="rnav">
    ...
    <div class="ph_news">
      ...
      <ul class="ph_n">
      <?php $i=1;foreach ($like_rows as $like_row):?>
        <li><span class="num1"><?php echo $i;?></span><a href="getDetail.php?id=<?php echo $like_row['id']?>" title=<?php echo $like_row['title'];?>></a></li>
   <?php $i++;endforeach;?>
      </ul>
      <h2>
        <p>最新评论</p>
